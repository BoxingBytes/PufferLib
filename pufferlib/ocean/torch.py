...
class Shared_Pool(nn.Module):
    def __init__(self, env, cnn_channels=32, hidden_size=128):
        ...
        self.input_dim = self.num_classes*np.prod(env.single_observation_space.shape)
    ...
    def encode_observations(self, observations, state=None):
        observations = F.one_hot(observations.long(), self.num_classes).view(-1, self.input_dim).float()
        observations = observations.float().view(-1, self.input_dim) / self.num_classes
