...
class LSD(nn.Module):
    ...
    def phi_forward(self, observations, state=None):
        ...
        assert hidden.dtype == lstm_h.dtype == lstm_c.dtype
        assert hidden.device == lstm_h.device == lstm_c.device
    ...
    def encode_observations(self, observations, state=None):
        ...
        no time dimension (handled by LSTM wrappers).'''
        skills = state['skills'] 
        assert skills.shape[:-1] == observations.shape[:-1], \
            f"Shape mismatch: {skills.shape} vs {observations.shape}"
